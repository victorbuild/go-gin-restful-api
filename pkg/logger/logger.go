...
func LogDebug(api string, message string) {
	debugLog := fmt.Sprintf(`{"timestamp": "%s", "api": "%s", "level": "debug", "message": "%s"}`,
		time.Now().Format(time.RFC3339), api, message)
...
// LogInfo 記錄一般資訊（正常流程）
func LogInfo(api string, message string) {
	infoLog := fmt.Sprintf(`{"timestamp": "%s", "api": "%s", "level": "info", "message": "%s"}`,
		time.Now().Format(time.RFC3339), api, message)
...
func LogWarn(api string, message string) {
	warnLog := fmt.Sprintf(`{"timestamp": "%s", "api": "%s", "level": "warn", "message": "%s"}`,
		time.Now().Format(time.RFC3339), api, message)
...
func LogFatal(api string, err error) {
	fatalLog := fmt.Sprintf(`{"timestamp": "%s", "api": "%s", "level": "fatal", "message": "%s"}`,
		time.Now().Format(time.RFC3339), api, err.Error())
